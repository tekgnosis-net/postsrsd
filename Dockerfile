...
RUN adduser -S postsrsd \
    && apk add --no-cache postsrsd=$POSTSRSD_PACKAGE_VERSION netcat-openbsd \
    && sed -i 's/^\(\(unprivileged-user\|chroot-dir\) = "\).*"/\1"/' \
         /etc/postsrsd/postsrsd.conf \
    && sed -i 's/^socketmap = inet:localhost:10003$/socketmap = inet:0.0.0.0:10003/' \
         /etc/postsrsd/postsrsd.conf \
...
HEALTHCHECK --interval=30s --timeout=3s --retries=3 \
    CMD nc -z localhost 10003 || exit 1
CMD set -x; \
    umask 0077; \
